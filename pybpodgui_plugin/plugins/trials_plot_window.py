...
from pybpodgui_plugin.api.models.board.com.state_entry import StateEntry
...
class TrialsPlotWindow(BaseWidget):
	...
	def __add_event(self, timestamp, track_id, name):
		if self._last_event:
			self._last_event.end = timestamp

		self._last_event = self._events.add_event(
			timestamp, timestamp, track=track_id,
			title=name, color=self._list_of_states_colors[track_id]
		)
		self._events.value = timestamp
			...
			for message in recent_history:
				try:
					if isinstance(message, StateEntry):
						...
						self.__add_event(int(round(message.board_timestamp*1000)), message.state_id - 1, message.state_name)
