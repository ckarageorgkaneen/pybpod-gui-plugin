...
from bpodgui_plugin.models.setup.setup_window import SetupWindow
...
class SetupTreeNode(SetupWindow):
	...
	**Methods**
	...
	def create_treenode(self, tree):
		...
		self.node = tree.create_child(self.name, self.experiment.node, icon=QtGui.QIcon(conf.SUBJECT_SMALL_ICON))
	...
	def remove(self):
		...
		.. seealso::
			* Setup removal (dock window): :py:meth:`pycontrolgui.models.setup.setup_dockwindow.SetupDockWindow.remove`.
			* Setup removal (API): :meth:`pycontrolapi.models.board.setup_base.SetupBase.remove`.
		"""
